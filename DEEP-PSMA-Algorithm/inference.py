...
##        print(f.read())
    ...
    transform = psma_fixed_transform.GetInverse()

    output_psma_ttb_label_resampled = sitk.Resample(
        output_psma_ttb_label,                    # The image to transform
        input_fdg_pet,                    # Reference image (same space, spacing, etc.)
        transform,                # The transform
        sitk.sitkNearestNeighbor,          # Interpolator (sitkNearestNeighbor for label images)
        0.0,                      # Default pixel value
        output_fdg_ttb_label.GetPixelID()        # Output pixel type
    )

    output_psma_ttb_prob_resampled = sitk.Resample(
        output_psma_ttb_prob,                    # The image to transform
        input_fdg_pet,                    # Reference image (same space, spacing, etc.)
        transform,                # The transform
        sitk.sitkLinear,          # Interpolator (sitkNearestNeighbor for label images)
        0.0,                      # Default pixel value
        sitk.sitkFloat32        # Output pixel type
    )

    ttb_FDG_filtered = filter_fdg_by_overlap_with_probabilities(
                                    psma_pred_sitk=output_psma_ttb_label_resampled,
                                    fdg_pred_sitk=output_fdg_ttb_label,                            
                                    prob_sitk_psma=output_psma_ttb_prob_resampled,
                                    prob_sitk_fdg=output_fdg_ttb_prob,
                                    )

    write_sitk_image_file(
        location=OUTPUT_PATH / "images/fdg-pet-ttb",
        image=ttb_FDG_filtered,
    )
